...
def _parse_ideal_conformer(mol_cif: cif_dict.CifDict) -> rd_chem.Conformer:
  ...
  atom_x = [
      ...
      for x in mol_cif['_chem_comp_atom.pdbx_model_Cartn_x_ideal']
  ]
  atom_y = [
      float(y) if y != "?" else 0.0
      for y in mol_cif['_chem_comp_atom.pdbx_model_Cartn_y_ideal']
  ]
  atom_z = [
      float(z) if z != "?" else 0.0
      for z in mol_cif['_chem_comp_atom.pdbx_model_Cartn_z_ideal']
  ]
