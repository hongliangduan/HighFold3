...
class TokenFeatures:
  ...
  def compute_features(
    ...
    # start of add on 20241126
    #print(all_tokens.shape[0])
    #print(all_tokens)
    #print(residue_index)
    #print(token_index)
    #print(seq_length)
    #cyc_matrix = construct_cyclic_offset_matrix(all_tokens.shape[0])
    #print(cyc_matrix)
    # end of add on 20241126
    
    # start of add on 20250228
    #print(all_tokens.shape[0])
    #index_residue_1 = np.where(residue_index == 1)[0]
    #n_res_in_ligand = index_residue_1[1] if len(index_residue_1) > 1 else len(residue_index)
    #cyc_matrix = construct_cyclic_offset_matrix(n_res_in_ligand)
    #print(cyc_matrix)
    # end of add on 20250228
    
    # start of add on 20250310
    cyc_matrix_residue = cyclic_peptide_matrix.calc_cyclic_matrix_af3(residue_index,
                                                                      head_to_tail=head_to_tail,
                                                                      list_cid_ss=list_cid_ss,
                                                                      token_wise=False,
                                                                      signal_bug_fix=True)
    cyc_matrix_residue = np.array(cyc_matrix_residue, dtype=np.int32)
    print(cyc_matrix_residue)
    cyc_matrix_token = cyclic_peptide_matrix.calc_cyclic_matrix_af3(residue_index,
                                                                    head_to_tail=head_to_tail,
                                                                    list_cid_ss=list_cid_ss,
                                                                    token_wise=True,
                                                                    signal_bug_fix=True)
    cyc_matrix_token = np.array(cyc_matrix_token, dtype=np.int32)
    print(cyc_matrix_token)
    # end of add on 20250310
    ...
    return TokenFeatures(
        ...
        cyc_matrix_residue=cyc_matrix_residue,
        cyc_matrix_token=cyc_matrix_token,
